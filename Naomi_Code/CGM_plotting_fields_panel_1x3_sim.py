...
Otherwise, no changes between v2 and original.

It will be hard-coded for HI+Xray with obslimits.

7/30/23: Adding accuracy.py script for plots.
'''
...
def create_subplot(fig, field_type, train, test, i, n_subplot):
    ...
    for j in range(len(params_true_[indexes,i])):
        ...
        chi2 = chi_squared(params_true_[:, i] - field_renorm[i], params_NN_[:, i] - field_renorm[i], errors_NN_[index,i])
        all_RMSE_values.append(RMSE)
        all_R2_values.append(R2)
        all_rmerr_values.append(RM_err)
        all_chi2_values.append(chi2)
        #ax.plot([],[],label='RMSE = {}'.format(RMSE))
        #ax.legend()
        ...
        if n_subplot == 1:
            ...
            #ax.axes.text(11.5, 14.25, r'$\rm{HI+Xray}$', fontsize=16)
            if i == 1:
            ...
            if i == 3:
                ...
                ax.set_xlabel(r'$\log(T_{\rm{cgm, truth}})$', fontsize=18)
            if i == 4:
...
# Xray Plots
ax1 = create_subplot(fig, 'HI_XraySoft', sims[0], sims[0], i, 1)
ax2 = create_subplot(fig, 'HI_XraySoft', sims[1], sims[1], i, 2)
ax3 = create_subplot(fig, 'HI_XraySoft', sims[2], sims[2], i, 3)
...
print('Label:', label)
fig.savefig('/home/ng474/CMD_py/panel_cut_1x3_sim_%s%s.png'%(label,obs_limit), bbox_inches='tight', pad_inches=0.25)
