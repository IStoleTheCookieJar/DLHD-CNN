...
#    if(i<1000): continue
    ...
    if(plot_stack):
        ...
        if(plot_glossy):
            ...
            if(plot_halo_mass_top):
                if(plot_labels):
                    #ax.text(64, 4, r"${\bf M_{\rm halo} = 10^{%5.2f}}$"%Mhalo[haloID],color="white", fontsize=18, horizontalalignment='center', verticalalignment='top',weight='bold')
                    #ax.text(64, 4, r"${\bf log(M_{\rm halo}/{\rm M_{\odot}} = %5.2f}$"%Mhalo[haloID],color="white", fontsize=18, horizontalalignment='center', verticalalignment='top',weight='bold')
                    #ax.text(4, 64, r"%s"%label,color="white", rotation="vertical", fontsize=18, horizontalalignment='left', verticalalignment='center',weight='bold')
                    pass
            else:
                if(plot_labels):
                    pass
                    #ax.text(64, 124, r"%s"%label ,color="white", fontsize=18, horizontalalignment='center', verticalalignment='bottom',weight='bold')
                    #ax.text(115, 120, r"%s"%label ,color="white", fontsize=18, horizontalalignment='right', verticalalignment='bottom',weight='bold')
                    #ax.text(4, 64, r"${\bf log(M_{\rm halo}/{M_{\odot}}) = %5.2f}$"%Mhalo[haloID],color="white", rotation="vertical", fontsize=14, horizontalalignment='left', verticalalignment='center',weight='bold')
                    #ax.text(64, 4, r"${\bf log(M_{\rm halo}/{M_{\odot}}) = %5.2f}$"%Mhalo[haloID],color="white", rotation="horizontal", fontsize=14, horizontalalignment='center', verticalalignment='top',weight='bold')
                    #ax.text(115, 4, r"%s"%sim, color="white", fontsize=18, horizontalalignment='right', verticalalignment='top', weight='bold')
        ...
        if("halocentric" in cut):
            ...
            if(sim=="TNG300"):
                ...
                fig.savefig("halo_maps/fig14/halo_%s.simID_%d.halo_%d.axis_%d%s.png"%(field,simID,haloID,axis,obslimit_suffix))
    ...
    if(plot_frame):
        ...
        if("halocentric" in cut):
            fig.savefig("halo_maps/testing/halo_%s.simID_%d.halo_%d.axis_%d.png"%(field,simID,haloID,axis))
