...
#             ,"M500c_","M200c","f_gas_0.5R200c","f_gas_0.7R200c","f_gas_R200c","f_gas_2R200c"
    # TNG100: "ID","M200c","f_gas_R500c", "a_form","conc_200c","shape","Log_M_star","M500c",
...
#-----------------
param_titles = []
params = np.zeros((len(ids_new),num_params))
...
rand_start = np.arange(int(len(DO_new["ID"])/3))
...
split_train = int(len(rand_start)*0.8)
split_test = len(rand_start)-split_train
print(f"training split looks like {split_train} galaxies \ntesting split looks like {split_test} galaxies")

new_train = np.empty(split_train*3)
new_test = np.empty(split_test*3)
new_train[:split_train] = rand_start[:split_train]*3
new_train[split_train:split_train*2] = rand_start[:split_train]*3+1
new_train[split_train*2:split_train*3] = rand_start[:split_train]*3+2
new_test[:split_test] = rand_start[split_train:]*3
new_test[split_test:split_test*2] = rand_start[split_train:]*3+1
new_test[split_test*2:split_test*3] = rand_start[split_train:]*3+2
new_train = new_train.astype(int)
new_test = new_test.astype(int)


print("Training indeces = ",new_train,"\nTesting indeces = ",new_test)
print(f"Shape of train = {new_train.shape}\nShape of test = {new_test.shape}")
...
for i in range(images.shape[1]):
    merged_x[ids_new[i]] = torch.tensor(
        images[:,i],
        dtype=torch.float32
    )
...
clid_test = []
for i in new_train: clid_train.append(ids_new[i])
for i in new_test: clid_test.append(ids_new[i])
print(clid_train[0])
...
study_name = sim+channels+param_str+"_Scatter"
...
def objective(trial):
    ...
    ####################### Cross Validation ###############################
    rand_mid = rand_start[:split_train]
    np.random.shuffle(rand_mid)
    
    split_train_mid = int((len(rand_mid))*0.8)
    split_test_mid = len(rand_mid)-split_train_mid
    train_indeces = np.empty(split_train_mid*3)
    valid_indeces = np.empty(split_test_mid*3)
    clid_train_mini = [] # flattened string cluster ids
    clid_valid = []
    Ntrain = split_train_mid
    Nvalid = int(split_train_mid/4)
    
    train_indeces[:split_train_mid] = rand_mid[:split_train_mid]*3
    train_indeces[split_train_mid:split_train_mid*2] = rand_mid[:split_train_mid]*3+1
    train_indeces[split_train_mid*2:split_train_mid*3] = rand_mid[:split_train_mid]*3+2
    valid_indeces[:split_test_mid] = rand_mid[split_train_mid:]*3
    valid_indeces[split_test_mid:split_test_mid*2] = rand_mid[split_train_mid:]*3+1
    valid_indeces[split_test_mid*2:split_test_mid*3] = rand_mid[split_train_mid:]*3+2
    train_indeces = train_indeces.astype(int)
    valid_indeces = valid_indeces.astype(int)
    
    
    for i in train_indeces: clid_train_mini.append(ids_new[i])
    for i in valid_indeces: clid_valid.append(ids_new[i])
    
#     random.shuffle(clid_train_mini)
#     random.shuffle(clid_valid)
    train_loader = build_loader(clid_train_mini, shuffle=False)
    valid_loader = build_loader(clid_valid, shuffle=False)
...
# TODO
    ...
    for i in range(N_epochs): # testing the model every 20 epochs of the pretraining model
        ...
        with torch.no_grad():
            for x, y, sizes, ids in tqdm(valid_loader, disable=not verbose):
    ...
    ids = []
    with torch.no_grad(): 
        for x, y, sizes, ide in tqdm(test_loader,disable=True):
            model.eval()
            pred = model.forward(x)
            ys.append(y)
            for i in range(num_params):
    ...
    if(len(study.get_trials())>1):
        if(scatter<study.best_trial.value):
...
#         plt.show()
#         f.tight_layout();
#         f.savefig(figure_directory + "Trial_"+trial_number+"_"+param_titles[i]+"_Scatter_TvP.png");
#         f.savefig("Trial_"+trial_number+"_"+param_titles[i]+"_Scatter_TvP.png");
#         plt.close(f);
        
    if(np.isnan(scatter)):
        scatter = np.inf
    
    return scatter # line fitting
...
study = optuna.create_study(study_name = study_name, storage=storage_name_db, direction="minimize"\
                           ,load_if_exists=True) #  and scatter
num_trials = 14
...
plt.ylabel('Loss')
plt.legend();
plt.savefig(figure_directory + "Trial_"+trial_number+"_Scatter_Loss.png")
plt.savefig("Trial_"+trial_number+"_Scatter_Loss.png")
...
with torch.no_grad():
    for x, y, sizes, ide in tqdm(test_loader):
        model.eval()
...
data_frame.to_csv(data_directory+"CNN_Light_Trial_"+trial_number+"_data_Scatter.csv")
