...
if(sys.argv[4] == "300"):
    ...
    Names_100 = ["ID","M200c","f_gas_R500c","a_form","conc_200c","shape","f_gas_R200C"\
                 ,"Log_M_star","Log_T","Log_Z","O/Fe","Ne/Fe","C/O","axis placeholder"\
                 ,"M500c","M200c_","f_gas_0.5R200c","f_gas_0.7R200c","f_gas_R200c","f_gas_2R200c"]
    Dataset_overlord = pd.read_csv("/pl/active/CASA/beop5934/halos/TNG100/Params_Groups_TNG100_z=0.00.ascii_ID_fgas"\
...
study_name = sim+channels+param_str+"_Double"
...
#------------------------------
def objective(trial):
    hidd = trial.suggest_int("hidden",1,3) # Hyper parameters to change
...
# TODO
    ...
    for i in range(N_epochs): # testing the model every 20 epochs of the pretraining model
        for x, y, sizes, ids in tqdm(train_loader, disable=not verbose):
            ...
            trloss += np.mean(loss.detach().flatten().numpy())
        with torch.no_grad():
            for x, y, sizes, ids in tqdm(test_loader, disable=not verbose):
                ...
                teloss += np.mean(loss.detach().flatten().numpy())
    ...
    with torch.no_grad(): 
        for x, y, sizes, ide in tqdm(all_loader,disable=True):
            ...
            for i in range(num_params):
                ...
                pred[:,i+num_params] += 2*np.log(maxs[i] - mins[i]) # gaussian
    ...
    for i in range(num_params):
        ...
        ax.errorbar(ys[:,i],preds[:,i],yerr = np.exp(preds[:,i+num_params]), fmt = '.', alpha = 0.1, color = "Green") # Gaussian PDF
        ...
        if(len(study.get_trials())>1):
            lr_found = study.best_trials[0].params['lr']
            wd_found = study.best_trials[0].params['wd']
        else:
            lr_found = Lr
            wd_found = W_d
        ...
        plt.show()
        f.tight_layout()
        f.savefig(figure_directory + "Trial_"+trial_number+"_"+param_titles[i]+"_Double_TvP.png");
        f.savefig("Trial_"+trial_number+"_"+param_titles[i]+"_Double_TvP.png");
        
    scatter = np.array(scatter)
    r_value = np.array(r_value)
    scatter = np.mean(scatter)
    ...
    if(len(study.get_trials())>1):
        if( (scatter<study.best_trials[0].values[0]) and (r_value>study.best_trials[0].values[1]) ):
    ...
    return scatter,r_value # line fitting




##################RUN MODEL#######################
study = optuna.create_study(study_name = study_name, storage=storage_name_db, directions=["minimize","maximize"]\
                           ,load_if_exists=True) #  and r_value
# study = optuna.create_study(direction='minimize') # test loss
num_trials = 20
...
#---------------------------
model = CNN(hidden=study.best_trials[0].params['hidden'],drop=study.best_trials[0].params['dr'],\
...
if(len(glob.glob(storage_name_pt))>0):
    ...
    if(int(study.best_trials[0].number) == epoch):
        same = True
...
if(same): ### if the best model run hasn't changed
    ...
    epoch = int(study.best_trials[0].number)
    starter = len(study.get_trials())-1
    model.load_state_dict(checkpoint_array[int(study.best_trials[0].number)-starter]['model'])
    optimizer.load_state_dict(checkpoint_array[int(study.best_trials[0].number)-starter]['optimizer'])
...
checkpoint_new = {
    ...
    'model' : deepcopy(model.state_dict()),
    'optimizer' : deepcopy(optimizer.state_dict()),
    'hidden' : study.best_trials[0].params['hidden'],
    'starter' :  starter
}
torch.save(checkpoint_new,storage_name_pt) ### saving whatever comes out
...
#-------------------------------------------
print('Best trial:')
print(f"\tnumber: {study.best_trials[0].number}")
print(f"\tparams: {study.best_trials[0].params}")
print(f"\tvalues: {study.best_trials[0].values}")
...
plt.ylabel('Loss')
plt.legend();
plt.savefig(figure_directory + "Trial_"+trial_number+"_Double_Loss.png")
plt.savefig("Trial_"+trial_number+"_Double_Loss.png")
...
# data_output = open(data_directory+"CNN_Light_"+ date +"_"+trial_number+".txt",'w')
# data_output.write("Pred\tError\tTruth   Id\n")
# for i in range(len(preds[:,0])):
#     data_output.write(f"{preds[i,0]:.3f}\t{preds[i,1]:.3f}\t{ys[i]:.3f}\t{ids[i]}\n")
# data_output.write(f'scatter: {np.std(preds[:,0]-ys):.3f} dex');
# data_output.close()
